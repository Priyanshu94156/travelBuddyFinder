...
(function ($) {
    ...
    if($(window).width()>768)
    {
        var defaultConfig = {
        num: 3, //要显示的数量，应该是个奇数
        maxWidth: $(window).width()*.60, //代表中央图片的宽度
        maxHeight: 285, //中央图片的高度
    ...
    else
    {
        ...
        var defaultConfig = {
        num: 1, //要显示的数量，应该是个奇数
        maxWidth: $(window).width()*.90, //代表中央图片的宽度
    ...
    function initStyle($container) {
        ...
        $container.find('.review_desc').css({
            width: $('.slideshow li.active').width()*.55
        });
